...
use super::structural::{fuzzy_structural_query, StructuralResult};
...
pub fn multi_hop_query(
    ...
    if relations.len() == 1 {
        return single_hop(start_entity, relations[0], atom_memory, composite_memory, triple_store, roles, admission, beta, k, max_iter);
    }

    if relations.len() == 2 {
        if let Some(rule) = rule_store.find_rule(relations[0], relations[1]) {
            return rule_rewrite(start_entity, &rule.output_relation, &rule.name, atom_memory, composite_memory, triple_store, roles, admission, beta, k, max_iter);
        }
    }

    chained_lookup(start_entity, relations, atom_memory, triple_store)
}
...
fn single_hop(
    ...
    let s_vec = match atom_memory.get(start_entity) {
        Some(v) => v,
        None => return Vec::new(),
    };
    let (_, r_vec) = atom_memory.get_or_insert(relation);

    let results = fuzzy_structural_query(
        atom_memory, composite_memory, triple_store, roles,
        &[("subject", &s_vec), ("relation", &r_vec)],
        "object", admission, beta, k, max_iter,
    );

    results.into_iter().map(|r| MultiHopResult {
...
fn rule_rewrite(
    ...
    let (_, r_vec) = atom_memory.get_or_insert(output_relation);
    ...
    results.into_iter().map(|r| MultiHopResult {
        entity_id: r.entity_id.clone(),
        confidence: r.confidence,
        method: MultiHopMethod::RuleRewrite { rule_name: rule_name.to_string() },
        hops: vec![HopDetail {
            from_entity: start_entity.to_string(),
...
fn chained_lookup(
    ...
    current_entities.into_iter().zip(all_hops.into_iter()).map(|(entity, hops)| {
        MultiHopResult {
            entity_id: entity,
            confidence,
...
mod tests {
    ...
    fn test_multi_hop_chained() {
        ...
        let results = multi_hop_query(
            "john", &["father", "father"],
            &atom_mem, &comp_mem, &triple_store, &roles, &rule_store,
            &admission, 24.0, 64, 3, 10,
        );
    ...
    fn test_multi_hop_rule_rewrite() {
        ...
        assert!(!results.is_empty());
        assert_eq!(results[0].entity_id, "bob");
        assert!(matches!(results[0].method, MultiHopMethod::RuleRewrite { .. }));
    }
}
