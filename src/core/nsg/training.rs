...
pub fn train(
    ...
    let id_map: Vec<(String, usize)> = ids
        .iter()
        .zip(offsets.iter())
...
mod tests {
    ...
    fn train_basic() {
        ...
        let offsets: Vec<usize> = (0..30).map(|i| i * 100).collect();
        ...
        let index = train(&vectors, &ids, &offsets, 1000, &config).unwrap();
    ...
    fn online_insert() {
        ...
        let offsets: Vec<usize> = (0..20).map(|i| i * 100).collect();
        let config = NSGConfig {
            max_degree: 8,
            ef_search: 32,
            ef_construction: 16,
        ...
        let mut index = train(&vectors, &ids, &offsets, 1000, &config).unwrap();
        let new_vec = EntangledHVec::new_deterministic(1000, 999);
        index.insert("new_vec", &new_vec, 9999).unwrap();
