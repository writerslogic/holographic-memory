...
fn measure_flat(dim: usize, denom: usize, items: &[EntangledHVec], probes: &[EntangledHVec], load_points: &[usize]) {
    ...
    for &n in load_points {
        ...
        let ms: Vec<f64> = items[..n].iter().map(|i| i.corrected_containment(&bundle)).collect();
        ...
        println!("flat\t{}\t{:.6}\t{:.6}\t{:.6}\t{:.6}\t{:.6}", n, mm, mi, nm, nx, mi - nx);
        if density > 0.995 { break; }
    }
    println!();
}

fn measure_cls(dim: usize, denom: usize, items: &[EntangledHVec], probes: &[EntangledHVec],
               load_points: &[usize], consol_interval: usize, heart1_cap: usize) {
    println!("# CLS dim={} denom={} consolidation_interval={} heart1_capacity={}",
    ...
    for i in 0..items.len() {
        ...
        if next_lp < load_points.len() && n == load_points[next_lp] {
            ...
            println!("cls\t{}\t{}\t{}\t{:.4}\t{:.4}\t{:.6}\t{:.6}\t{:.6}\t{:.6}\t{:.6}",
                     n, mem.heart1_count(), mem.heart2_count(),
                     mem.heart1_density(), mem.heart2_density(),
                     mm, mi, nm, nx, mi - nx);
        }

        if n >= *load_points.last().unwrap_or(&0) { break; }
    }
    println!();
}

fn mean(v: &[f64]) -> f64 { v.iter().sum::<f64>() / v.len() as f64 }
fn fmin(v: &[f64]) -> f64 { v.iter().cloned().fold(f64::INFINITY, f64::min) }
fn fmax(v: &[f64]) -> f64 { v.iter().cloned().fold(f64::NEG_INFINITY, f64::max) }
