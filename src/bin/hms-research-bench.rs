...
fn main() {
    ...
    if !json_output {
        eprintln!(
            "HMS Research Benchmark — D={}, density=1/{}",
            dim, density_denom
        );
        eprintln!("=========================================================");
    }
...
fn bench_knowledge_graph(dim: usize, dd: usize, quick: bool) -> serde_json::Value {
    ...
    for (i, &(true_rel, true_a, true_b)) in all_facts.iter().enumerate().take(n_test) {
        ...
        let best_r = (0..all_relations.len())
            ...
            .unwrap();
        if *relation_names[best_r] == true_rel {
            rel_correct += 1;
        }
    }
    ...
    for &(rel, ref pairs) in &kg.relations {
        ...
        for i in 0..pairs.len() {
            for j in (i + 1)..pairs.len() {
                ...
                let candidates: Vec<&&str> = entity_vecs
                    .keys()
                ...
                let best = candidates
                    ...
                    .max_by(|&&cand, &&other| {
                        entity_vecs[*cand]
                            .permute(role_arg2)
                            .containment_similarity(&relation_bundle)
                            .partial_cmp(
                                &entity_vecs[*other]
                                    .permute(role_arg2)
                                    .containment_similarity(&relation_bundle),
    ...
    for &n_facts in &fact_counts {
        ...
        for t in 0..n_facts {
            let (a_idx, b_idx) = if t % 3 == 0 {
                (
                    shared_entity,
                    1 + (hash_u64(0x1A00, t as u64) as usize % (n_entities - 1)),
                )
            } else if t % 3 == 1 {
                (
                    1 + (hash_u64(0x1B00, t as u64) as usize % (n_entities - 1)),
                    shared_entity,
                )
            } else {
            ...
            let best_a = (0..n_entities)
                .max_by(|&x, &y| {
                    entities[x]
                        .permute(role_arg1)
                        .containment_similarity(comp)
                        .partial_cmp(&entities[y].permute(role_arg1).containment_similarity(comp))
                        .unwrap()
                })
                .unwrap();
            if best_a == true_a {
            ...
            let best_b = (0..n_entities)
                .max_by(|&x, &y| {
                    entities[x]
                        .permute(role_arg2)
                        .containment_similarity(comp)
                        .partial_cmp(&entities[y].permute(role_arg2).containment_similarity(comp))
                        .unwrap()
                })
                .unwrap();
            if best_b == true_b {
        ...
        let shared_facts = facts
            .iter()
            .filter(|f| f.0 == shared_entity || f.1 == shared_entity)
            .count();
        ...
        let all_bindings: Vec<EntangledHVec> = facts
            .iter()
            .flat_map(|&(a, b)| {
                vec![
        ...
        for &(true_a, true_b) in &facts[..n_probe] {
            let best_a = (0..n_entities)
                .max_by(|&x, &y| {
                    entities[x]
                        .permute(role_arg1)
                        .containment_similarity(&mega_bundle)
                        .partial_cmp(
                            &entities[y]
                                .permute(role_arg1)
                                .containment_similarity(&mega_bundle),
...
fn bench_multihop(dim: usize, dd: usize) -> serde_json::Value {
    ...
    let capital_map: HashMap<&str, &str> = kg
        .relations
        .iter()
        .find(|r| r.0 == "capital_of")
        .unwrap()
        .1
        .iter()
        .copied()
        .collect();
    let continent_map: HashMap<&str, &str> = kg
        .relations
        .iter()
        .find(|r| r.0 == "continent_of")
        .unwrap()
        .1
        .iter()
        .copied()
    ...
    let all_entities: Vec<(&str, &EntangledHVec)> =
        entity_vecs.iter().map(|(&k, v)| (k, v)).collect();
    ...
    for (&country, &capital) in &capital_map {
        if let Some(&continent) = continent_map.get(country) {
            ...
            let best_capital = all_entities
                .iter()
                .max_by(|a, b| {
                    a.1.permute(role_arg2)
                        .containment_similarity(&fact_cap)
                        .partial_cmp(&b.1.permute(role_arg2).containment_similarity(&fact_cap))
                        .unwrap()
                })
                .unwrap();

            if best_capital.0 == capital {
                hop1_correct += 1;
            }
            ...
            let best_cont = all_entities
                .iter()
                .max_by(|a, b| {
                    a.1.permute(role_arg2)
                        .containment_similarity(&fact_cont)
                        .partial_cmp(&b.1.permute(role_arg2).containment_similarity(&fact_cont))
                        .unwrap()
                })
                .unwrap();

            if best_cont.0 == continent {
                hop2_correct += 1;
            }
            total += 1;
        }
    ...
    let chains: Vec<(&str, &str, &str, &str)> = capital_map
        .iter()
        .filter_map(|(&country, &capital)| {
            continent_map
                .get(country)
                .map(|&cont| (country, capital, country, cont))
        })
        .collect();
    ...
    for &(country, capital, _, continent) in &chains {
        ...
        let retrieved_capital = all_entities
            .iter()
            .max_by(|a, b| {
                a.1.permute(role_arg2)
                    .containment_similarity(&fact1)
                    .partial_cmp(&b.1.permute(role_arg2).containment_similarity(&fact1))
                    .unwrap()
            })
            .unwrap();

        if retrieved_capital.0 == capital {
            let retrieved_cont = all_entities
                .iter()
                .max_by(|a, b| {
                    a.1.permute(role_arg2)
                        .containment_similarity(&fact2)
                        .partial_cmp(&b.1.permute(role_arg2).containment_similarity(&fact2))
                        .unwrap()
                })
                .unwrap();

            if retrieved_cont.0 == continent {
                chain_correct += 1;
            }
        }
    }
...
fn bench_sequence(dim: usize, dd: usize, quick: bool) -> serde_json::Value {
    ...
    for &seq_len in &test_lengths {
        ...
        for trial in 0..n_trials {
            ...
            let bindings: Vec<EntangledHVec> = sequence
                .iter()
                .enumerate()
                ...
                let best = (0..n_vocab)
                    .max_by(|&x, &y| {
                        vocab[x]
                            .permute(pos + 1)
                            .containment_similarity(&comp)
                            .partial_cmp(&vocab[y].permute(pos + 1).containment_similarity(&comp))
                            .unwrap()
                    })
                    .unwrap();
                if best == true_idx {
...
fn bench_head_to_head(hrr_dim: usize, ehv_dim: usize, dd: usize, quick: bool) -> serde_json::Value {
    ...
    for i in 0..4 {
        ehv_roles.push(EntangledHVec::new_with_density(
            ehv_dim,
            dd,
            0x40BE + i as u64,
        ));
    ...
    for &rel_name in test_relations {
        let pairs: &Vec<(&str, &str)> = &kg.relations.iter().find(|r| r.0 == rel_name).unwrap().1;
        ...
        for &(a, b) in pairs {
            ...
            let ehv_best = kg
                ...
                .max_by(|&&x, &&y| {
                    ehv_entities[x]
                        .permute(3)
                        .containment_similarity(&ehv_fact)
                        .partial_cmp(&ehv_entities[y].permute(3).containment_similarity(&ehv_fact))
                        .unwrap()
                })
                .unwrap();
            if *ehv_best == b {
            ...
            let hrr_best = kg
                ...
                .max_by(|&&x, &&y| {
                    decoded
                        .cosine(&hrr_entities[x])
                        .partial_cmp(&decoded.cosine(&hrr_entities[y]))
                        .unwrap()
                })
                .unwrap();
            if *hrr_best == b {
    ...
    for &seq_len in &seq_lengths {
        ...
        for trial in 0..n_trials {
            ...
            let ehv_bindings: Vec<EntangledHVec> = sequence
                .iter()
                .enumerate()
            ...
            let hrr_bindings: Vec<HrrVec> = sequence
                .iter()
                .enumerate()
            ...
            for (pos, &true_idx) in sequence.iter().enumerate() {
                let ehv_best = (0..n_seq_vocab)
                    .max_by(|&x, &y| {
                        ehv_vocab[x]
                            .permute(pos + 1)
                            .containment_similarity(&ehv_comp)
                            .partial_cmp(
                                &ehv_vocab[y]
                                    .permute(pos + 1)
                                    .containment_similarity(&ehv_comp),
                ...
                let hrr_best = (0..n_seq_vocab)
                    .max_by(|&x, &y| {
                        hrr_decoded
                            .cosine(&hrr_vocab[x])
                            .partial_cmp(&hrr_decoded.cosine(&hrr_vocab[y]))
                            .unwrap()
                    })
                    .unwrap();
                if hrr_best == true_idx {
                    hrr_correct += 1;
                }
...
fn print_report(report: &serde_json::Value) {
    ...
    eprintln!(
        "D={}, density=1/{}, active={}, elapsed={:.1}s",
        meta["dim"],
        meta["density_denom"],
        meta["active_indices"],
    ...
    eprintln!(
        "    {} real + {} synthetic = {} total facts, {} entities, {} relations",
        kg["n_real_facts"],
        kg["n_synthetic_facts"],
        kg["total_facts"],
        kg["unique_entities_in_facts"],
        kg["n_relations"]
    );
    eprintln!(
        "    Max entity frequency: {} (tests Zipfian interference)",
        kg["max_entity_frequency"]
    );
    eprintln!(
        "    Arg1 accuracy:     {:.1}%",
        kg["arg1_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    eprintln!(
        "    Arg2 accuracy:     {:.1}%",
        kg["arg2_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    eprintln!(
        "    Relation accuracy: {:.1}%",
        kg["relation_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    eprintln!(
        "    Overall:           {:.1}%",
        kg["overall_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );

    let an = &report["analogy"];
    eprintln!("\n[2] ANALOGY COMPLETION (A:B :: C:?)");
    eprintln!(
        "    {} analogies tested, {:.1}% accuracy",
        an["total_analogies"],
        an["accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    ...
    eprintln!(
        "\n[3] INTERFERENCE STRESS ({} entities, shared entity across facts)",
        intf["n_entities"]
    );
    eprintln!(
        "    {:>6}  {:>10}  {:>10}  {:>12}",
        "Facts", "Individual", "Bundled", "Bun.Density"
    );
    ...
    eprintln!(
        "\n[4] MULTI-HOP INFERENCE ({} countries)",
        mh["n_countries_tested"]
    );
    eprintln!(
        "    Hop 1 (country→capital):  {:.1}%",
        mh["hop1_capital_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    eprintln!(
        "    Hop 2 (country→continent): {:.1}%",
        mh["hop2_continent_accuracy"].as_f64().unwrap_or(0.0) * 100.0
    );
    eprintln!(
        "    Chain (capital via country→continent): {:.1}%",
